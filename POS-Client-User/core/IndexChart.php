...
// STATIONARY
    // Retail Stationary
    $cate = '5';
    $sqle = "SELECT COUNT(delivery_mechant_log_id_fk) AS Rstation FROM pos_delivery_calls,mechants WHERE delivery_mechant_log_id_fk = mechant_log_id AND mechant_type = 'Retail' AND delivery_category_id_fk=? AND delivery_call_month=? AND delivery_call_year = ?;";
    $stmte = $object->connect()->prepare($sqle);
    $stmte->bindvalue(1,$cate);
    $stmte->bindvalue(2,date('F'));
    $stmte->bindvalue(3,date('Y'));
    ...
    $cate = '5';
    $sqle = "SELECT COUNT(delivery_mechant_log_id_fk) AS Fstation FROM pos_delivery_calls,mechants WHERE delivery_mechant_log_id_fk = mechant_log_id AND mechant_type = 'Forecourt' AND delivery_category_id_fk=? AND delivery_call_month=? AND delivery_call_year = ?;";
    $stmte = $object->connect()->prepare($sqle);
    $stmte->bindvalue(1,$cate);
    $stmte->bindvalue(2,date('F'));
    $stmte->bindvalue(3,date('Y'));
