<?php
    ...
    class Categories extends Database
    {
        ...
        function NewCategory($category)
        {
            try
            {
                $sql = "INSERT INTO pos_categories(category,created_by,date_created) VALUES(?,?,?);";
        ...
        function DeleteCategory($categoryID)
        {
            try
            {
                $sql = "DELETE FROM pos_categories WHERE category_id = ?;";
                $stmt = $this->connect()->prepare($sql);
    ...
    class SubCategory extends Database
    {
        ...
        function NewSubCategory($category,$sub_category)
        {
            try
            {
                $sql = "INSERT INTO pos_sub_categories(category_id_fk,sub_category,created_by,date_created) VALUES(?,?,?,?);";
                $stmt = $this->connect()->prepare($sql);
                $stmt->bindvalue(1, $category);
        ...
        function DeleteSUBCategory($sub_categoryID)
        {
            try
            {
                $sql = "DELETE FROM pos_sub_categories WHERE sub_category_id = ?;";
                $stmt = $this->connect()->prepare($sql);
