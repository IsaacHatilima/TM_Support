<?php
    if(isset($_GET['client']))
    {
        ...
        // New Calls
        $state = 'New';
        $sqle1 = "SELECT COUNT(call_id) AS totas1 FROM atm_calls WHERE call_status = ? AND client_id_fk=? AND log_year=? ;";
        ...
        // Pending Calls
        $statepen = 'Open';
        $sqlpen1 = "SELECT COUNT(call_id) AS totas2 FROM atm_calls WHERE call_status = ? AND client_id_fk=? AND log_year=?";
        ...
        // Closed Calls
        $stateclo = 'Closed';
        $sqlclo1 = "SELECT COUNT(call_id) AS totas3 FROM atm_calls WHERE call_status = ? AND client_id_fk=? AND log_year=?";
        ...
        // PIE CHART 2
        // Hardware
        $sqle = "SELECT COUNT(call_id) AS hardware FROM atm_calls,atm_categories WHERE category_id=category_id_fk AND  category LIKE '%Hardware' AND client_id_fk = ? AND log_year = ?;";
        ...
        //Software
        $cate = '2';
        $sqle = "SELECT COUNT(call_id) AS software FROM atm_calls,atm_categories WHERE category_id=category_id_fk AND  category LIKE '%Software' AND client_id_fk = ? AND log_year = ?;";
        $stmte = $object->connect()->prepare($sqle);
        $stmte->bindvalue(1,$client_id);
        $stmte->bindvalue(2,$year);
        if($stmte->execute())
