			...
			<div class="content-wrapper">
				<div class="panel panel-flat">
					<div class="panel-heading">
						<div class="row">
							<div class="col-md-6">
								<h6 class="panel-title">ATM Tickets</h6>
							</div>
							<div class="col-md-6">
										...
										<div class="row">
											...
											<div class="col-md-4">
												<div class="row">
													...
													<div class="col-md-8">
													<input type="text" class="form-control" data-mask="9999-99-99" placeholder="Enter Starting Date" required="required" name="start_date">
		                        	                <span class="help-block">9999-99-99</span>
													</div>
												</div>
											</div>
											<div class="col-md-4">
												<div class="row">
													...
													<div class="col-md-8">
													<input type="text" class="form-control" data-mask="9999-99-99" placeholder="Enter End Date" required="required" name="end_date">
		                        	                <span class="help-block">9999-99-99</span>
													</div>
												</div>
											</div>
												...
												<tbody>
													<?php
														if(isset($_GET['client']))
														{
															...
															{
																echo "1";
																$sql = "SELECT * FROM atm_calls f,clients,atm_details, client_users,atm_categories,atm_sub_categories,engineers WHERE client_id=f.client_id_fk AND atm_id=atm_id_fk AND client_user_id = logged_by AND category_id=f.category_id_fk AND sub_category_id=sub_category_id_fk AND engineer_id=engineer_on_site AND f.client_id_fk=? AND time_logged LIKE  '$start%'  AND call_status='Closed';";
															}
															else
															{
																$sql = "SELECT * FROM atm_calls f,clients,atm_details, client_users,atm_categories,atm_sub_categories,engineers WHERE client_id=f.client_id_fk AND atm_id=atm_id_fk AND client_user_id = logged_by AND category_id=f.category_id_fk AND sub_category_id=sub_category_id_fk AND engineer_id=engineer_on_site AND f.client_id_fk=? AND CAST(time_logged as DATE) BETWEEN '$start%'  AND '$end%' AND call_status='Closed';";
															}
												...
												<tbody>
													<?php
														if(isset($_GET['client']))
														{
															...
															{
																$sql = "SELECT * FROM atm_calls f,clients,atm_details, client_users,atm_categories,atm_sub_categories,engineers WHERE client_id=f.client_id_fk AND atm_id=atm_id_fk AND client_user_id = logged_by AND category_id=f.category_id_fk AND sub_category_id=sub_category_id_fk AND engineer_id=engineer_on_site AND f.client_id_fk=? AND CAST(time_logged as DATE) BETWEEN '$start%'  AND '$end%' AND call_status='Open';";
															}
															$stmt = $object->connect()->prepare($sql);
															$stmt->bindvalue(1, $client_id);
															$stmt->execute();
												...
												<tbody>
													<?php
														if(isset($_GET['client']))
														{
															...
															{
																$sql = "SELECT * FROM atm_calls f,clients,atm_details, client_users,atm_categories,atm_sub_categories,engineers WHERE client_id=f.client_id_fk AND atm_id=atm_id_fk AND client_user_id = logged_by AND category_id=f.category_id_fk AND sub_category_id=sub_category_id_fk AND engineer_id=engineer_on_site AND f.client_id_fk=? AND CAST(time_logged as DATE) BETWEEN '$start%'  AND '$end%' AND call_status='New';";
															}
															$stmt = $object->connect()->prepare($sql);
															$stmt->bindvalue(1, $client_id);
															$stmt->execute();
				...
				<!-- Pie Charts -->
				<?php include '../core/DateRange.php'; ?>
				<div class="panel panel-flat">
					<div class="panel-heading">
						...
						if(isset($client_id))
						{
							...
							if($stmt->rowCount())
							{
								while ($rows = $stmt->fetch())
								{
									if($start == $end )
									{
										$sql1 = "SELECT COUNT(call_id) AS firsts FROM atm_calls f,atm_details WHERE f.client_id_fk=? AND atm_id_fk=atm_id AND province=? AND time_logged LIKE '$start%';";
									}
									else
									{
										$sql1 = "SELECT COUNT(call_id) AS firsts FROM atm_calls f,atm_details WHERE f.client_id_fk=? AND atm_id_fk=atm_id AND province=? AND CAST(time_logged as DATE) BETWEEN '$start%'  AND '$end%';";
									}
									
									$stmt1 = $object->connect()->prepare($sql1);
									$stmt1->bindvalue(1, $client_id);
									$stmt1->bindvalue(2, $rows['province']);
									$stmt1->execute();
									$rows1 = $stmt1->fetch();
						...
						if(isset($client_id))
						{
							...
							if($stmt->rowCount())
							{
								while ($rows = $stmt->fetch())
								{
									if($start == $end )
									{
										$sql2 = "SELECT COUNT(call_id) AS tots FROM atm_calls WHERE logged_by=? AND client_id_fk=? AND time_logged LIKE '$start%';";
									}
									else
									{
										$sql2 = "SELECT COUNT(call_id) AS tots FROM atm_calls WHERE logged_by=? AND client_id_fk=? AND CAST(time_logged as DATE) BETWEEN '$start%'  AND '$end%';";
									}
