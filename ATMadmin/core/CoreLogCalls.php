<?php
    ...
    class Calls extends Database
    {
        ...
        function deviceNewCall($call_priority,$mechant_log_id_fk,$call_device_serial,$category_id_fk,$sub_category_id_fk,$fault_details,$managers_name,$managers_cell,$logged_by,$date_loged,$mechtype,$clientID)
        {
            ...
            try
            {
                ...
                $status = "New";
                $sql = "INSERT INTO pos_device_calls(ticket_number,call_priority,devcall_mechant_log_id_fk,call_device_serial,category_id_fk,sub_category_id_fk,fault_details,managers_name,managers_cell,logged_by,date_loged,device_call_status,call_month,call_year,mecha_type,device_qota,clientID,logger) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
                $stmt = $this->connect()->prepare($sql);
                ...
                $stmt->bindvalue(18, $_SESSION['person']);
                if($stmt->execute())
                {
        ...
        function sendEmail($logged_by,$ticket_number)
        {
            ...
            try
            {
                ...
                $stmte->execute();
                $liste = array();
                $liste =  $stmte->fetchAll(PDO::FETCH_ASSOC);
                //$output = array();
                ...
                // Ticket Number   
                // Engineers
                $sqleng = "SELECT engineer_first_name,engineer_last_name FROM engineers WHERE engineer_id= ? ;";
                $stmteng = $this->connect()->prepare($sqleng);
                $stmteng->bindvalue(1, $_SESSION['person']);
                $stmteng->execute();
                $roweng = $stmteng->fetch();
                $email = $rowclientID['email'];
                $message_subject = 'Notification For Ticket '.sprintf("%04d", $ticket_number).'.';
                                            ...
                                            <table border="1" cellpadding="0" cellspacing="0" style="border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; width: 100%;">
                                                ...
                                                <tr>
                                                    <td align="left"
                                                        style="font-family: sans-serif; vertical-align: top">
                                                        Logged BY: </td>
                                                    <td class="lines">'.$roweng ['engineer_first_name'].' '.$roweng ['engineer_last_name'].'</td>
                                                </tr>
                                                <tr>
                                                    <td align="left"
                                                        style="font-family: sans-serif; vertical-align: top">
        ...
        function deliveryNewCall($delivery_call_priority,$delivery_mechant_log_id_fk,$delivery_category_id_fk,$delivery_sub_category_id_fk,$item_to_deliver,$delivery_managers_name,$delivery_managers_cell,$delivery_logged_by,$delivery_date_loged,$delivery_mechtype,$clientID)
        {
            $sql = "SELECT ticket_number FROM ticket_numbers;";
            $stmt = $this->connect()->prepare($sql);
            $stmt->execute();
            ...
            try
            {
                ...
                $stmt->bindvalue(17, $_SESSION['person']);
                if($stmt->execute())
                {
                    $sql1 = "UPDATE ticket_numbers SET ticket_number = ?;";
                    $stmt1 = $this->connect()->prepare($sql1);
                                        ...
                                        Ticket Number '.sprintf("%04d", $rowdets['ticket_number']).'  <br> <br>
                                            <tr>
                                                ...
                                                <table border="1" cellpadding="0" cellspacing="0" style="border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; width: 100%;">
                                                    ...
                                                    <tr>
                                                        <td class="lines" align="left"
                                                            style="font-family: sans-serif; font-size: 14px; vertical-align: top">Logged BY</td>
                                                        <td class="lines">'.$roweng ['engineer_first_name'].' '.$roweng ['engineer_last_name'].'</td>
                                                    </tr>
                                                    <tr>
                                                        <td class="lines" align="left"
    ...
    if(!empty($_POST['dev_serial']))
    {
        ...
        $stmte1->bindvalue(1,$logged_by);
        $stmte1->execute();
        $row = $stmte1->fetch(PDO::FETCH_ASSOC);
        $clientID = $row['client_id_fk'];

        $tikets -> deviceNewCall($call_priority,$mechant_log_id_fk,$call_device_serial,$category_id_fk,$sub_category_id_fk,$fault_details,$managers_name,$managers_cell,$logged_by,$date_loged,$mechtype,$clientID);
    }
    ...
    if(!empty($_POST['mech_id']))
    {
        ...
        $sqle1 = "SELECT client_id_fk FROM client_users WHERE client_user_id = ?;";
        $stmte1 = $object->connect()->prepare($sqle1);
        $stmte1->bindvalue(1,$delivery_logged_by);
        $stmte1->execute();
        $row = $stmte1->fetch(PDO::FETCH_ASSOC);
        $clientID = $row['client_id_fk'];

        $tikets -> deliveryNewCall($delivery_call_priority,$delivery_mechant_log_id_fk,$delivery_category_id_fk,$delivery_sub_category_id_fk,$item_to_deliver,$delivery_managers_name,$delivery_managers_cell,$delivery_logged_by,$delivery_date_loged,$delivery_mechtype,$clientID);
    }
